...
var hosts = []string{"http://129.154.205.7:7195", "https://windsurf.jeter.eu.org"}
var host = hosts[1]
var githubPath = "https://ghp.ci/https://github.com/kingparks/windsurf-vip/releases/download/latest/"
var err error
...
var jbProduct = []string{"Windsurf IDE"}
...
func Run() (productSelected string, modelIndexSelected int) {
	...
	fmt.Printf(green, Trr.Tr("Windsurf VIP")+` v`+strings.Join(strings.Split(fmt.Sprint(version), ""), "."))
	Cli.SetProxy(lang)
	checkUpdate(version)
...
func getPromotion() (promotion string) {
	b, _ := os.ReadFile(os.Getenv("HOME") + "/.windsurf-viprc")
